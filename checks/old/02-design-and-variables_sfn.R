...
processed_box_id <- tracking_sheet[this_dataset_row, "processed_box_file_ID"]
qa_box_folder_id <-
...
sheet1_required_cols <- c(1, 2, 3, 4, 5, 7, 10, 11)
...
if (all_sheets$sheet2$`Is it available?`[1]) {
  ...
  if (any(!(sheet7_plants_plots$in_sheet6), na.rm = T)) {
    error_list <-
      c(error_list,
...
box_write(tracking_sheet, "sfn_tracking.xlsx", "230431401206")
