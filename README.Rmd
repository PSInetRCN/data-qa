...
sfn_data  <- box_read_excel("1451006536192", sheet = 1) |>
  filter(submission_path != "foo")
...
checks1 <- tracking_data |>
  group_by(completed_checks_1, submission_path) |>
  tally() |>
...
checks2 <- tracking_data |>
  group_by(completed_checks_2, submission_path) |>
  tally() |>
...
ggplot(checks, aes(check_level, n, fill = check_status)) +
  ...
  facet_wrap(vars(submission_path), ncol = 1)
