...
sheet1$latitude_wgs84 <- substr(sheet1$latitude_wgs84, 1, 9)
...
sheet7 <- sheet7 |>
  mutate(time = ifelse(time == "predawn", "06:00:00", "12:00:00"),
         water_potential_mean = paste0(-1, water_potential_mean))
...
flag_summary <- "SW units not provided, met values out of range, note positive WP values changed to negative"
