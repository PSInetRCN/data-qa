...
sheet7 <- sheet7 |>
  tidyr::separate_wider_delim(time, "-", names = "time2", too_many = "drop", cols_remove = F) |>
  mutate(time2 = paste0(time2, ":00")) |>
  mutate(time_num = ifelse(grepl(":", time),
                         NA,
                         as.numeric(time))) |>
  ...
  mutate(time = ifelse(
    is.na(time_POSIX),
    time2,
    format(time_POSIX, format = "%H:%M:%S"))) |>
  select(-time_num,-time_seconds,-time_POSIX, -time2) |>
  mutate(time = ifelse(nchar(time) < 8,
                       paste0("0", time),
                       time))
...
flag_summary <- "Plant in WP not in plants, met values out of range, missing expected variable"
