...
sfn_site <- "ESP_GUA_VAL" # Change this to match each site
...
site_md$Study_description[2] <- "Single-site experiment"
...
treatmentCols <- blank_psinet_template[[4]][0, ]

treatmentVals <- data.frame(
  `Level of treatment` = "Individual",
  `Treatment ID` = unique(sfn_wp$pl_treatment)
)
